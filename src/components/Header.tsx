...
interface HeaderProps {
    ...
    onNavigateToNotificationLink: (link: Notification['link']) => void;
}
    ...
    <>
      <header className="bg-white shadow-sm p-4 flex items-center justify-between sticky top-0 z-20 h-20 border-b border-slate-200">
          <div className="flex items-center min-w-0">
               <button onClick={onToggleSidebar} className="text-slate-600 hover:text-green-700 focus:outline-none mr-4">
                  {isSidebarOpen ? ICONS.close : ICONS.menu}
              </button>
              <div className="min-w-0">
                  <h1 className="text-lg sm:text-2xl font-bold text-slate-800 truncate">Nwangele LGA Loan Management</h1>
                  <p className="text-sm text-slate-500 hidden sm:block">Empowering Local Businesses in Imo State</p>
              </div>
          </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
          {showSearchBar && (
              <div className="relative hidden sm:block">
                <input
                  ...
                  className="w-40 md:w-56 lg:w-64 pl-4 pr-10 py-2 border border-slate-300 rounded-full focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 focus:w-48 md:focus:w-72"
                  value={searchTerm}
          ...
          <div className="relative" ref={notificationRef}>
            <button
              ...
              className="p-2 rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
            ...
            {isNotificationsOpen && (
              <NotificationPanel
                ...
                onNavigate={onNavigateToNotificationLink}
          ...
          <div className="relative" ref={profileRef}>
            <div className="flex items-center space-x-2">
              ...
              <button onClick={() => setProfileOpen(!isProfileOpen)} className="hidden sm:block text-left focus:outline-none">
                <div className="font-semibold text-slate-700 hover:text-green-600">{currentUser?.name}</div>
            ...
            {isProfileOpen && (
              <div className="absolute top-full right-0 mt-3 w-72 bg-white rounded-xl shadow-2xl border animate-fade-in origin-top-right overflow-hidden">
                <div className="p-4 flex items-center gap-4 border-b bg-slate-50">
                ...
                <div className="p-4 space-y-2 text-sm">
                  ...
                  {currentUser?.ward && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-slate-700">{currentUser.ward}</span>
                    </div>
                  )}
                </div>
                <div className="p-2 border-t">
                   <a
                      ...
                      className="flex items-center gap-3 w-full text-left px-3 py-2 rounded-md text-slate-700 hover:bg-slate-100"
                    >
