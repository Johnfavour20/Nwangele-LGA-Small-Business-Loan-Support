...
export const NewApplicationForm: React.FC<NewApplicationFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const getInputClasses = (fieldName: keyof FormErrors) => {
      ...
      return errors[fieldName]
        ...
        : `${base} border-gray-300 focus:ring-green-500`;
  };

  return (
    <div className="bg-white p-4 sm:p-8 rounded-xl shadow-md max-w-4xl mx-auto border border-slate-200">
      <h2 className="text-3xl font-bold text-gray-800 mb-2">New Loan Application</h2>
      <p className="text-gray-600 mb-8">Follow the steps below to complete your application.</p>
      ...
      <form onSubmit={handleSubmit}>
        {currentStep === 1 && (
          <div className="space-y-6 animate-fade-in">
            <h3 className="text-xl font-semibold text-gray-700">Step 1: Business Details</h3>
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label htmlFor="sector" className="block text-sm font-medium text-gray-700 mb-1">Business Sector</label>
        ...
        {currentStep === 2 && (
          <div className="space-y-6 animate-fade-in">
            <h3 className="text-xl font-semibold text-gray-700">Step 2: Purpose & Description</h3>
             <div>
              <label htmlFor="businessDescription" className="block text-sm font-medium text-gray-700 mb-1">Detailed Business Description</label>
             ...
             <div>
              <label htmlFor="loanPurpose" className="block text-sm font-medium text-gray-700 mb-1">Purpose of the Loan</label>
        ...
        {currentStep === 3 && (
            <div className="animate-fade-in">
                <h3 className="text-xl font-semibold text-gray-700 mb-4">Step 3: Upload Documents</h3>
                <div className={`border-2 border-dashed rounded-lg p-8 text-center bg-gray-50 hover:border-green-500 transition ${errors.files ? 'border-red-500' : 'border-gray-300'}`}>
                ...
                {files.length > 0 && (
                    <div className="mt-6">
                        ...
                        {files.map((file, index) => (
                            <li key={index} className="flex items-center justify-between p-2 bg-gray-100 rounded-md text-sm">
                                <span>{file.name}</span>
        ...
        {currentStep === 4 && (
            <div className="space-y-6 animate-fade-in">
                <h3 className="text-xl font-semibold text-gray-700">Step 4: Review Your Application</h3>
                <div className="bg-gray-50 p-6 rounded-lg space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><p className="text-sm text-gray-500">Business Name</p><p className="font-semibold">{formData.businessName}</p></div>
                        <div><p className="text-sm text-gray-500">Business Sector</p><p className="font-semibold">{formData.sector}</p></div>
                        <div><p className="text-sm text-gray-500">Loan Amount</p><p className="font-semibold">₦{Number(formData.loanAmount).toLocaleString()}</p></div>
                    </div>
                     <div className="pt-4 border-t"><p className="text-sm text-gray-500">Business Description</p><p>{formData.businessDescription}</p></div>
                     <div className="pt-4 border-t"><p className="text-sm text-gray-500">Loan Purpose</p><p>{formData.loanPurpose}</p></div>
                ...
                <p className="text-sm text-gray-600">By submitting this application, you confirm that all information provided is accurate and complete.</p>
            </div>
        )}
        ...
        <div className="mt-10 flex justify-between items-center border-t pt-6">
          ...
          <div className="flex items-center space-x-4">
            <button type="button" onClick={onCancel} disabled={isSubmitting} className="text-sm font-semibold text-gray-600 hover:text-gray-800 disabled:opacity-50">Cancel</button>
