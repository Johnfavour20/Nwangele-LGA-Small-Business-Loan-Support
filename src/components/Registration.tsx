...
export const Registration: React.FC<RegistrationProps> = ({ onSwitchToLogin }) => {
    ...
    const labelClasses = "font-semibold text-slate-700 dark:text-slate-300 block mb-1";
        ...
        <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center">
            <div className="w-full max-w-4xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex overflow-hidden">
                ...
                <div className="w-full md:w-1/2 p-8 sm:p-12">
                    <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-2">Create an Account</h2>
                    <p className="text-slate-600 dark:text-slate-400 mb-6">Get started with your application today.</p>
                    
                    <form onSubmit={handleRegister} className="space-y-4">
                        <div>
                            <label className={labelClasses}>Full Name</label>
                            <input type="text" name="name" value={formData.name} onChange={handleChange} className={inputClasses} placeholder="e.g., Adaobi Ekwueme" required />
                        </div>
                         <div>
                        ...
                        <div>
                            <label className={labelClasses}>Ward</label>
                             <select name="ward" value={formData.ward} onChange={handleChange} className={`${inputClasses} bg-white dark:bg-slate-700`}>
                                {LGA_WARDS.map(ward => <option key={ward} value={ward}>{ward}</option>)}
                             </select>
                        </div>
                        <div>
                            <label className={labelClasses}>Password</label>
                            <input type="password" name="password" value={formData.password} onChange={handleChange} className={inputClasses} placeholder="••••••••" required />
                        </div>
                        <div>
                            <label className={labelClasses}>Confirm Password</label>
                            <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} className={inputClasses} placeholder="••••••••" required />
                        </div>
